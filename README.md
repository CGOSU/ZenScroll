...
- **`ScrollConfig`** — 可调物理参数：摩擦力(`friction`)、回弹力(`bounce_tension`)、加速度(`scroll_accel`)、最大/最小速度
...
| 应用 | 摩擦力 (Friction) | 回弹力 (Bounce) | 加速度 (Accel) | 最大速度 (MaxV) | 最小速度 (MinV) | 匹配进程 |
|------|------------------|-----------------|----------------|-----------------|-----------------|----------|
| Chrome | 0.94 | 0.85 | 1.5 | 200 | 0.30 | chrome.exe, msedge.exe, brave.exe, opera.exe |
| Readest | 0.96 | 0.90 | 1.0 | 120 | 0.20 | readest.exe |
| Firefox | 0.93 | 0.85 | 1.3 | 180 | 0.40 | firefox.exe |
...
每次物理 tick 将速度乘以该系数。**值越大，滚动越滑、越持久**（如 0.96 的 Readest 预设手感绵长）；**值越小，滚动越"涩"、停得越快**（如 0.93 的 Firefox 预设更干脆）。接近 1.0 时滚动几乎不减速，低于 0.90 时滚动明显顿挫。
